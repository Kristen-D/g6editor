...
function cronValidate(cronExpression ){
  ...
  if (cronParams.length < 6 || cronParams.length > 7) {
  ...
  }else if (cronParams[cronParams.length-1]=='') {
    return 'cron表达式不能以空格结尾';
  }else{
    //日和周必须有一个为?，或者全为*
    if((cronParams[3] == "?" && cronParams[5] != "?") || (cronParams[5] == "?" && cronParams[3] != "?") || (cronParams[3] == "*" && cronParams[5] == "*")){
      ...
      return "指定第4位(日)时第6位(周)必须设为不指定(?),指定第6位(周)时第4位(日)必须设为不指定(?)"
    }
  }
}
...
function checkSecondsField(secondsField) {
  return checkField(secondsField, 0, 59, "第1位(秒)");
}

//检查分的函数方法
function checkMinutesField(minutesField) {
  return checkField(minutesField, 0, 59, "第2位(分)");
}

//检查小时的函数方法
function checkHoursField(hoursField) {
  return checkField(hoursField, 0, 23, "第3位(时)");
}
...
function checkDayOfMonthField(dayOfMonthField) {
  ...
  if (dayOfMonthField.indexOf("L") >= 0) {
      return checkFieldWithLetter(dayOfMonthField, "L", 1, 7, "第4位(日)");
  } else if ( dayOfMonthField.indexOf("W") >= 0) {
      return checkFieldWithLetter(dayOfMonthField, "W", 1, 31, "第4位(日)");
  } else if (dayOfMonthField.indexOf("C") >= 0) {
      return checkFieldWithLetter(dayOfMonthField, "C", 1, 31, "第4位(日)");
  }
  return checkField( dayOfMonthField, 1, 31, "第4位(日)");
}
...
function checkMonthsField(monthsField) {
  ...
  if(monthsField != "*"){
    ...
    return checkField(monthsField, 1, 12, "第5位(月)");
  }else{
    return true;
  }
...
function checkDayOfWeekField(dayOfWeekField) {
  ...
  if (dayOfWeekField.indexOf("L") >= 0) {
      return checkFieldWithLetterWeek(dayOfWeekField, "L", 1, 7, "第6位(周)");
  } else if (dayOfWeekField.indexOf("C") >= 0) {
      return checkFieldWithLetterWeek(dayOfWeekField, "C", 1, 7, "第6位(周)");
  } else if (dayOfWeekField.indexOf("#") >= 0) {
      return checkFieldWithLetterWeek(dayOfWeekField, "#", 1, 7, "第6位(周)");
  } else {
      return checkField(dayOfWeekField, 1, 7, "第6位(周)");
  }
}

//检查年份的函数方法
function checkYearField(yearField) {
  return checkField(yearField, 1970, 2099, "第7位(年)");
}
