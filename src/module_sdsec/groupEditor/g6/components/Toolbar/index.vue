<template>
    ...
    ></i>
      ...
      <i data-command="paste" class="command iconfont icon-paster-o disable" title="粘贴"></i>-->
        ...
        <el-button @click="consoleData" type="primary" size="small">控制台输出数据</el-button>
       <!-- <el-button @click="saveImg" type="primary" size="small">保存图像</el-button>
        <el-button @click="test" type="primary" size="small">测试</el-button>-->
    ...
    export default {
        ...
        methods: {
            ...
            bindEvent() {
                ...
                eventBus.$on("updatePb", async (pbId) =>{
                    let vo = {};
                    vo.id = pbId;
                    vo.flowXml =  JSON.stringify(this.graph.save());
                    let baseResult = await this.utils.reqObjBackState("/ssa/soar/playbook/updatePlaybook.do",vo);
                    let data = baseResult.data;
                    if(!data.state){
                        this.$message({message : data.message,type : 'info'});
                    }
                });
            },
            handleUndo() {
            ...
            test() {
                debugger;
                const nodeIds = ['node2','node7', 'node16'];
                const nodes = [{id:'node2',state:'over'},{id:'node7',state:'error'}, {id:'node16',state:'error'},];
                nodes.forEach(node => { debugger
                    const item = this.graph.findById(node.id);
                    var state = node.state;
                    this.graph.setItemState(item, state, true);
                    const edges = item.getEdges();
                    edges.forEach(edge => {
                        if (nodeIds.indexOf(edge.getSource()._cfg.id)>-1&&nodeIds.indexOf(edge.getTarget()._cfg.id)>-1) {
           ...
           /* saveImg() {
             ...
             // debugger
               /!* if (true) {
                  this.graph.fitView(20); // 图自动缩放以适应画布尺寸
                }*!/
