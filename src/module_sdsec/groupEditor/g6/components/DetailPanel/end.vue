            ...
            >
              <el-table-column prop="code" label="参数" align="center" width="140">
                <template slot-scope="scope">
                  <el-input placeholder="code" :disabled="true" v-model="scope.row.code"></el-input>
                </template>
              </el-table-column>
  ...
  watch : {
    node : {
      async handler(newValue,oldValue){
        let params = {};
        params.pbId = this.node.pbId;
        params.id = this.node.id;
        this.options = await this.utils.reqObj('/ssa/soar/node/getSrNodeValTree.do',params);
      },
      deep : true,
    }
  },
