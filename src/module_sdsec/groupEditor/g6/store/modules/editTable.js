...
const actions = {
  ...
  async loadScriptData({
    ...
    if (!params) params = state[id].params;
    if (!url) url = state[id].url;
    params.pageSize = state[id].pageSize;
    params.pageIndex = state[id].pageIndex;
    var gridData = await common.req(url, params);
    commit('setScriptAfterLoad', [id,gridData]);
    return gridData;
  },
  ...
  async reloadScript({
    ...
    var data = await dispatch('loadScriptData',[id]);
    return data;
  },
  ...
  async handleDelete({
    ...
    context.$confirm('此操作将删除该行, 是否继续?', '提示', {
      ...
      var msg = await common.req(state[id].delUrl, {
        ids: ids
      }, context);
...
const mutations = {
    ...
    setScriptAfterLoad(state, [id,data]) {
      state[id].scriptData = data.rowData;
    },
    cancelEdit(state, id) {
