...
const actions={
  async loadStrategyConfigurationTable({commit,state},[id,params,url]){
  ...
  async reloadTable({dispatch,state},id) {
    ...
    var data = await dispatch('loadStrategyConfigurationTable', [id]);
    return data;
  },
  async handleCurrentChange({dispatch,state},[id,pageIndex]){
    state[id].pageIndex = pageIndex;
    return await dispatch('loadStrategyConfigurationTable',[id]);
  },
