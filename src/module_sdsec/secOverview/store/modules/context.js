...
const actions={
  async loadSecurityOverviewTable({commit,state},[id,params,url]){
  ...
  async reloadTable({dispatch,state},id) {
    ...
    var data = await dispatch('loadSecurityOverviewTable', [id]);
    return data;
  },
  async handleCurrentChange({dispatch,state},[id,pageIndex]){
    state[id].pageIndex = pageIndex;
    return await dispatch('loadSecurityOverviewTable',[id]);
  },
