...
const actions={
  async loadOperationOverviewTable({commit,state},[id,params,url]){
  ...
  async reloadTable({dispatch,state},id) {
    ...
    var data = await dispatch('loadOperationOverviewTable', [id]);
    return data;
  },
  async handleCurrentChange({dispatch,state},[id,pageIndex]){
    state[id].pageIndex = pageIndex;
    return await dispatch('loadOperationOverviewTable',[id]);
  },
