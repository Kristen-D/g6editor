...
<script>
  import {fmtEventRiskList, fmtProductTypeList,
  fmtEventTypeList, fmtEventThreatList, fmtEventGradeList} from "../../js/functionUtil";
  ...
  export default {
    ...
    async created () {
      ...
      this.eventGradeList = await fmtEventGradeList(this);
      this.eventRiskList = await fmtEventRiskList(this);
      this.productTypeList = await fmtProductTypeList(this);
    ...
    methods: {
      append1() {
        ...
        for (let i = 0; i < keyData.length; i++) {
          for (let key in this.tableData) {//取出map对象的值
            if (this.tableData[key] != null && this.tableData[key] != '' && key == keyData[i].name) {
              ...
              if (key == "parentEventType_s" || key == "eventType_s") {
                child["value"] = this.eventTypeList[this.tableData[key]];
              } else if (key == "riskLevel_s") {
                child["value"] = this.eventRiskList[this.tableData[key]];
              } else if (key == "threatType_s") {
                child["value"] = this.eventThreatList[this.tableData[key]];
              } else if (key == "severityLevel_s") {
                child["value"] = this.eventGradeList[this.tableData[key]];
              } else {
      ...
      append2() {
        ...
        let keyData = [
          ...
          {"lname": "所属机构", "name": "org_name"},
          {"lname": "资产负责人", "name": "owner"},
          {"lname": "使用人", "name": "asset_user"},
          {"lname": "资产外部IP", "name": "assert_out_ip"},
        ...
        for (let i = 0; i < keyData.length; i++) {
          for (let key in this.tableData) {//取出map对象的值
            if (this.tableData[key] != null && this.tableData[key] != '' && key == keyData[i].name) {
              ...
              child["lname"] = keyData[i].lname;
              if (key == "product_type") {
                child["value"] = this.productTypeList[this.tableData[key]];
              } else {
                child["value"] = this.tableData[key];
              }
