...
<div>
  ...
  <el-tabs v-model="activeTable" @tab-click="handleClick">
    ...
    <el-tab-pane label="进行中" name="myTaskTable">
      <taskInspectMyTask :id="myTaskTable"></taskInspectMyTask>
    </el-tab-pane>
    <el-tab-pane label="已完成" name="myTaskHiTable">
      <taskInspectMyTask :id="myTaskHiTable"></taskInspectMyTask>
    </el-tab-pane>
  </el-tabs>
</div>
</template>
  ...
  methods: {
    ...
    var  rows = this.$store.state.table[this.activeTable].multipleSelection;
      ...
      this.$confirm('此操作将删除选中条目，是否继续？', '提示', {
        ...
        var taskInspectIds = [];
        rows.forEach(row => {
          taskInspectIds.push(row.task_inspect_id);
        });
        var msg = await this.utils.req("/ssa/taskInspect/deleteSelectedTaskInspect.do", {
          taskInspectIds: taskInspectIds
        }, this);
        this.$message({
          type: 'success',
          message: msg
