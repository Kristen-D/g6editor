...
<div>
  ...
  <el-tabs v-model="activeTable" @tab-click="handleClick">
    ...
    <el-tab-pane label="进行中" name="myTaskTable">
      <evaluationOrganMyTask :id="myTaskTable"></evaluationOrganMyTask>
    </el-tab-pane>
    <el-tab-pane label="已完成" name="myTaskHiTable">
      <evaluationOrganMyTask :id="myTaskHiTable"></evaluationOrganMyTask>
    </el-tab-pane>
  </el-tabs>
</div>
</template>
<script>
import evaluationOrganMyTask from './EvaluationOrganMyTask'
import toolbar from './Toolbar'
export default {
  components: {
    evaluationOrganMyTask,
    toolbar
  },
  data() {
    return {
  ...
  methods: {
    ...
    var  rows = this.$store.state.table[this.activeTable].multipleSelection;
      ...
      this.$confirm('此操作将删除选中条目，是否继续？', '提示', {
        ...
        var msg = await this.utils.req("/ssa/evaluationOrgan/detleEvaluationInfo.do", {
          ids: ids
        }, this);
        this.$message({
          type: 'success',
