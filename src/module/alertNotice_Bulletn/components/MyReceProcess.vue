...
<div class="whitebackground Whitespace">
  ...
  <el-tabs v-model="activeName" @tab-click="handleClick">
    <el-tab-pane :label="'未签收（共'+tableTotal1+'条）' " name="myReceInfoTable" v-if="this.hasRole(this.roles,[1,2,5,6,7,12])">
      <editTable :checkList="columnList" :id="tableId1"></editTable>
    </el-tab-pane>
    <el-tab-pane :label="'已签收（共'+tableTotal2+'条）'" name="myReceInfoTable1" v-if="this.hasRole(this.roles,[1,2,5,6,7,12])">
      <caimedTable :checkList="columnList" :id="tableId2"></caimedTable>
    </el-tab-pane>
    <el-tab-pane :label="'待处理（共'+tableTotal3+'条）'" name="recvTodoTask" v-if="this.hasRole(this.roles,[1,2,3,4,5,6,7])">
       <myRecvTodoTask :checkList="columnList" :id="tableId3" ></myRecvTodoTask>
    </el-tab-pane>
    <el-tab-pane :label="'已处理（共'+tableTotal4+'条）'" name="recvHisTask" v-if="this.hasRole(this.roles,[1,2,3,4,5,6,7])">
      <myRecvHisTask :checkList="columnList" :id="tableId4" ></myRecvHisTask>
    </el-tab-pane>
  </el-tabs>
</div>
</template>
...
import caimedTable from './NoticeInfoCaimedTable.vue'
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      tableTotal2:state => state.table["myReceInfoTable1"].total,
      tableTotal3:state => state.table["recvTodoTask"].total,
      tableTotal4:state => state.table["recvHisTask"].total,
    })
  },
  data() {
    return {
      activeName:'myReceInfoTable',
      activeTab: 'myReceInfoTable',
      tableId1: "myReceInfoTable",
      tableId2: "myReceInfoTable1",
      tableId3: "recvTodoTask",
      tableId4: "recvHisTask",
//      roles:[],
  ...
  methods: {
    ...
    async delCheckedRows() {
      ...
      var i;
      for (i in rows) {
        if (rows[i].status == 1 || rows[i].status == 3 || rows[i].status == 4 || rows[i].status == 5) {
