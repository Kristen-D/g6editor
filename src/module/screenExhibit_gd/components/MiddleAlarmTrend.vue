  ...
  export default {
    ...
    data(){
            return {
              ...
              detailTitle:"攻击详情",
              tableUrl:"22",
              detailTableId:"middleAlarmDetail",
              unitOptions: [{
                  label: '已处置'
                },
                {
    ...
    methods: {
      clCheckedRows(){
        ...
        this.$root.$children[0].form.pageIndex= this.$store.state.editTable[this.detailTableId].pageIndex;
          ...
          chartClick (params) {
            this.showDetail(params.data[0],params.seriesIndex,2);//日期，(已处置0,未处置1),中危
          },
          showDetail(dataTime,dataType,dataType2){
            this.detailTitle=""+dataTime+"的中危"+this.unitOptions[dataType].label+"告警详情";
...
this.publicShow=false;
    ...
    computed: {
      option () {
        return this.$store.state.context.middleAlarm;
      },
      total () {
        return `(${this.$store.state.context.alarmSmallTitle}: ${this.$store.state.context.middleAlarmTotal}条)`;
      }
    }
  }
</script>
