   ...
   var id = context.id;
     context.$confirm('此操作将删除该行, 是否继续?', '提示', {
       ...
       type: 'warning'
        ...
        var data = await common.reqDef(state[id].delUrl,{ids:ids}, context);
          context.$message({ type: 'success',   message: data.message });
        if(data.state){
          dispatch('reloadTable',id);
        }
...
const mutations = {
  ...
  setTableAfterLoad(state,[id,data]){
       ...
       state[id].loading = false;console.log(data.rowData);
