      ...
      <div class="box">
        ...
        <el-row :gutter="24">
          <el-col :span="17" style="position: relative">
            <e-charts :options="optionMap" @chartclick="chartClick" auto-resize></e-charts>
            <div class="btn-map">
              <el-button type="text" @click="mapChange('world')">
                <i class="icon_wd" :class="{active: smallWorld}"></i>
              </el-button>
              <el-button type="text" @click="mapChange('china')">
                <i class="icon_gd" :class="{active: smallGd}"></i>
              </el-button>
            </div>
          </el-col>
          <el-col :span="7">
            <e-charts :options="optionPie"  @chartclick="chartClickPie" auto-resize></e-charts>
            <!-- <e-charts style="height: 340px" :options="optionBar" @chartclick="chartClickBar" auto-resize></e-charts> -->
        ...
        this.$root.$children[0].form.pageIndex= this.$store.state.editTable[this.detailTableId].pageIndex;
          ...
          chartClickPie (params) {
            if(params.seriesIndex==0){
              if(params.name=="已完成"){
                this.showDetailAlarm(3,params.name);
              }else{
                this.showDetailAlarm(2,params.name);
              }
            }else{
              var ipStr="";
              for (var i = 0; i < this.$store.state.context.srcIpList.ipList.length; i++) {
         ...
         showDetailAlarm(dataType,str){
           this.detailTitle=""+str+"的告警详情";
           this.tableUrl="/ssa/screenExhibit/getAlarmInfoDetail.do";
           this.params.externalIp_s=this.$store.state.context.params.externalIp_s;
           this.params.timeValue=this.$store.state.context.params.timeValue;
           this.params.timeType=this.$store.state.context.params.timeType;
           this.params.dataType=dataType;
           this.params.dataType3=3;
           this.myShow = true;
           this.params = {...this.params};
        },
         onCloseIp() {
...
<style scoped>
  ...
  .btn-map .icon_gd {
    ...
    height: 57px;
    margin: 0 auto;
    background: url(/static/images/screen_gd/china-icon.png) no-repeat;
  }
