   ...
   var id = context.id;
     context.$confirm('此操作将删除该行, 是否继续?', '提示', {
       ...
       type: 'warning'
        ...
        var role_ids = [];
        role_ids.push(row.listId);
        var ids = role_ids.join(",");
        var msg = await common.reqBackState(state[id].delUrl,{id:ids}, context);
        //  await dispatch('reloadTable',id);
        if(msg){
          context.$message({ type: 'success',   message: msg.data });
          state[id].tableData.splice(i, 1);
        }else {
          context.$message({ type: 'error',   message: msg.data });
        }
         return msg ;
     })
 },
