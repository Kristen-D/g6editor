<template>
  <div>
    <div class="el-subpanel-heading" style="padding-left:15px;padding-top:15px;">
      <h2 class="lightPurple" v-if="this.$route.params.handle == 'add'&& this.$route.params.bw_type == 0 ">新增黑名单</h2>
      <h2 class="lightPurple" v-if="this.$route.params.handle == 'update'&& this.$route.params.bwType == 0 ">编辑黑名单</h2>
    </div>
    <div class="areaFocus el-button--purple">
      <div class="el-subpanel-body">
        <!-- <div>类型：{{this.type}}</div> -->
        <el-form ref="blackWhiteForm" :model="blackWhiteForm" :rules="rules" label-width="90px">
          ...
          <el-form-item label="内容" prop="list_value" style="width:500px">
            <el-input type="textarea" v-model="blackWhiteForm.list_value"></el-input>
          </el-form-item> -->
...
export default {
  ...
  methods: {
    ...
    async onSubmit() {
      ...
      // if (this.checkForm()) {
        var res = await utils.reqBackState("/ssa/blackWhiteList/saveData.do", this.blackWhiteForm, this);
        if(res){
          this.$message({
            type: "success",
            message: res.data
