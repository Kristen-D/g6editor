<template >
   ...
   <div class="el-panel-white-heading">
     <h3 class="el-panel-white-title">攻击类型趋势</h3>
...
export default {
  computed:{
   ...
   watch:{
    ...
    methods: {
      changeTime(value){
        if (value.name=="1"){
          this.loadMapChart("day");
        }else if(value.name=="2"){
          this.loadMapChart("week");
        }else if(value.name=="3"){
          this.loadMapChart("month");
        }
      ...
      async loadMapChart(dateRange){
        ...
        var data = await this.commonAjax.req("/ssa/situationAnaly/getLineByEventThreatType.do", this.params);
  ...
  color: ['#1B55A5', '#00B8FF', '#FDB502','#0066cc', '#00cccc','#6600cc','#6666cc','#66cccc','#336633','#33ff33','#9900ff','#9999ff'],
      ...
      var data2 = await this.commonAjax.req("/ssa/situationAnaly/getPieByRiskType.do", this.params);
            var seriesData2=[];
