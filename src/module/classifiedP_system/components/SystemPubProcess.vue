...
<div>
  ...
  <el-tabs v-model="activeTable" @tab-click="handleClick">
    ...
    <el-tab-pane label="进行中" name="myTaskTable">
      <systemMyTask :id="myTaskTable"></systemMyTask>
    </el-tab-pane>
    <el-tab-pane label="已完成" name="myTaskHiTable">
      <systemMyTask :id="myTaskHiTable"></systemMyTask>
    </el-tab-pane>
  </el-tabs>
</div>
</template>
<script>
import systemMyTask from './SystemMyTask.vue'
  ...
  methods: {
    ...
    var  rows = this.$store.state.table[this.activeTable].multipleSelection;
      ...
      this.$confirm('此操作将删除选中条目，是否继续？', '提示', {
        ...
        var  sysIds = [];
        rows.forEach(row =>{
          sysIds.push(row.sys_id);
        });
        var msg = await  this.utils.req("/ssa/classifiedP/DetleSystemInfo.do", {sysIds:sysIds}, this);
