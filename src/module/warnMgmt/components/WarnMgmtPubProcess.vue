...
<div>
  ...
  <el-tabs v-model="activeTable" @tab-click="handleClick">
    ...
    <el-tab-pane label="进行中" name="myTaskTable">
      <warnMgmtMyTask :id="myTaskTable"></warnMgmtMyTask>
    </el-tab-pane>
    <el-tab-pane label="已完成" name="myTaskHiTable">
      <warnMgmtMyTask :id="myTaskHiTable"></warnMgmtMyTask>
    </el-tab-pane>
  </el-tabs>
  ...
  <el-dialog title="导入" :visible.sync="importDialogVisible" width="30%">
    ...
    <a href="/static/template/WarnInfo.xlsx" class="dialog-downloadtext font-color">预警模版</a>
    <el-upload ref="upload" action="/ssa/warnMgmt/importExcel.do" :on-success="handleSuccess" :auto-upload="false">
...
<script>
import warnMgmtMyTask from './WarnMgmtMyTask'
import {
  mapState
} from 'vuex'
  ...
  methods: {
    ...
    async delCheckedRows() {
      ...
      this.$confirm('此操作将删除选中条目, 是否继续?', '提示', {
        ...
        var msg = await this.utils.req("/ssa/warnMgmt/delwarHandleInfo.do", {
          "alertIds": alertIds
        }, this);
        //alert(alertIds);
        this.$message({
