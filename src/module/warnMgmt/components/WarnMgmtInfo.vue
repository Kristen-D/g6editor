  ...
  <el-row>
    ...
    <el-col :span="24">
      <el-collapse v-model="activeNames" @change="showTaskDeTail($event)" class="Check-the-process-state-collapse">
        ...
        <el-collapse-item class="processcollapse-item" v-for="(item, index) in hiTasks" v-show="item.taskDefinitionKey != 'alertFbtask'" :name="item.processInstanceId+','+item.taskDefinitionKey+','+item.id">
...
export default {
  ...
  data() {
    return {
      ...
      detailMap: {
          warnHandleInfoTask:"warnMgmtDetail",
          alertTztask:"warnMgmtDetail",
          //alertFbtask:"warnMgmtFbForm",
          alertSptask:"warnMgmtAuditDetail",
          alertFktask:"warnMgmtFeedBackDetail",
          alertYztask:"warnMgmtYzDetail",
          alertYzSptask:"warnMgmtYzSpDetail",
          alertJctask:"warnMgmtJcDetail"
        }
    };
  },
  created: function() {
    ...
    this.utils.req("/ssa/warnMgmt/listHiTask.do", {
      processInstanceId: processInstanceId
    }, this).then(result => {
      this.hiTasks = result;
      // alert(this.hiTasks.length);
      //加入空数组用于显示原始表单数据
      this.hiTasks.push({
        name: "发布预警表单",
        taskDefinitionKey: "warnHandleInfoTask",
        processInstanceId: processInstanceId,
        id: this.$route.params.alert_id,
        endTime:this.$route.params.alert_time,
        assignee:this.$route.params.creator
      });
      // alert(this.hiTasks.length);
