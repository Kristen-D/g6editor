  ...
  <el-table  :data="defConf.tableData" v-loading="defConf.loading" style="width: 110%" @selection-change="handleSelectionChange">
    ...
    <el-table-column label="操作" align="center">
      <template scope="scope">
       ...
       <el-button  v-if="scope.row.status =='0' && scope.row.handle =='permitted' " type="text" class="operate-left" @click="handleEdit(scope.row)" >编辑</el-button>
...
export default {
  ...
  methods: {
    ...
    async handleAduit(row) {
      var result = await this.common.req("/ssa/technical/startProcess.do", row, this);
      this.$message({
        type: "success",
        message: result
      });
  ...
  created: function() {
    var defConf = {
      ...
      multipleSelection: [],
      //默认ajax属性
      tableUrl: "/ssa/technical/gettechnicalInfo.do",
      params: {},
      saveUrl: "/ssa/technical/saveTaskTechnical.do",
      delUrl: "/ssa/technical/deltechnicalInfo.do",
    };
