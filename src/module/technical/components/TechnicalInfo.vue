  ...
  <el-row>
    ...
    <el-col :span="24">
      <el-collapse v-model="activeNames" @change="showTaskDeTail($event)" class="Check-the-process-state-collapse">
        ...
        <el-collapse-item class="processcollapse-item" v-for="(item, index) in hiTasks" v-show="item.taskDefinitionKey != 'assistTjTask'" :name="item.processInstanceId+','+item.taskDefinitionKey+','+item.id">
...
export default {
  ...
  created: function() {
    ...
    this.assist_code = params.assist_code;
    //this.orgName = params.orgName;
    var processInstanceId = params.processInstanceId;
    this.processInstanceId = processInstanceId;
    this.common.req("/ssa/technical/listHiTask.do", {
      ...
      this.hiTasks.push({
        name: "协查详单",
        taskDefinitionKey: "technicalTask",
        processInstanceId: processInstanceId,
        id: this.assist_code,
    ...
    async showTaskDeTail(activeNames, e) {
      ...
      for (id in activeNames) {
        ...
        if (this.store[task] == undefined) {
          ...
          params.assist_code = this.assist_code;
