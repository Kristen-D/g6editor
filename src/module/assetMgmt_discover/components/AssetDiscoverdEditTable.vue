...
export default {
  ...
  methods: {
    ...
    handleEdit(i, row) {
      ...
      this.$router.push({name:'assetDiscoverdForm', params:row })
      // this.$store.dispatch('handleEdit', [this.id, i, row]);
    },
    handleDelete(i, row) {
      this.$store.dispatch('handleDelete', [this, i, {"assetIds":[row.assetId]}]);
    },
    saveEdit(i, row) {
  ...
  created: function() {
    var defConf = {
      ...
      multipleSelection: [],
      //默认ajax属性
      tableUrl: "/ssa/assetDiscover/queryAssetByDiscoverd.do",
      params: {},
      saveUrl: "/ssa/assetDiscover/saveDiscoverdAsset.do",
      delUrl: "/ssa/assetDiscover/deleteDiscoverdAssets.do",
    };
