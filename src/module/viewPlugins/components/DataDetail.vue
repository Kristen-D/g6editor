<template>
        ...
        <table class="dialog-table" width="100%">
          <tbody>
            ...
            <tr>
              ...
              <td colspan="5">{{ data.msg_s }}</td>
            </tr>

            <tr>
              <td colspan="1">信息描述</td>
              <td colspan="5"  style="white-space:pre-wrap;!important;word-break:break-all;">{{ data.msg_content }}</td>
              </tr>
          </tbody>
      </table>
     </div>
  ...
  import {mapState} from 'vuex'
      ...
      computed:{
       ...
       methods:{
         ...
         for(var i=0;i<keyData.length;i++){
            for(var key in this.data){//取出map对象的值
              if(this.data[key]!=null&&this.data[key]!=''&&key==keyData[i].name){
                var child={};
                child["name"]=key;
                child["lname"]=keyData[i].lname;
                if(key=="parentEventType_s"||key=="eventType_s"){
                  child["value"]=this.fmtEventTypeList(this.data[key]);
                }else if(key=="riskLevel_s"){
                  child["value"]=this.fmtEventRiskList(this.data[key]);
                }else if(key=="threatType_s"){
                  child["value"]=this.fmtEventThreatList(this.data[key]);
                }else if(key=="severityLevel_s"){
                  child["value"]=this.fmtEventGradeList(this.data[key]);
                }else{
    ...
    // <tr>
    //   <td rowspan="2"></td>
    //   <td rowspan="2" class="dialog-attack-number" :style="{color: data.event_check_score>90?'#03a182':(data.event_check_score>80?'#ff8400':'#f72e00')}">{{ data.event_check_score }}</td>
      //
      // <td >{{fmtPacketDirection(data.)}}</td>
      // <td colspan="3" style="overflow-x:auto;"> {{ data. }} </td>
        ...
        for(var key in this.data){//取出map对象的值
          if(this.data[key]!=null&&this.data[key]!=''&&key==keyData[i].name){
            ...
            if(key=="parentEventType_s"||key=="eventType_s"){
              ...
              child["value"]=this.fmtPacketDirection(this.data[key]);
            }else{
              child["value"]=this.data[key];
            }
     ...
     for(var i=0;i<keyData.length;i++){
        for(var key in this.data){//取出map对象的值
          if(this.data[key]!=null&&this.data[key]!=''&&key==keyData[i].name){
            ...
            child["lname"]=keyData[i].lname;
            if(key=="product_type"){
              child["value"]=this.fmtProductTypeList(this.data[key]);
            }else{
              child["value"]=this.data[key];
            }
