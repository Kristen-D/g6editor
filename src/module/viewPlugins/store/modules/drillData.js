...
const mutations = {
  openDrillDialogTable(state, params) {
    ...
    state.drillData.queryParams = {
      p1: '',
      p2: '',
      p3: ''
    };
    let moduleName = params.module;
    if(moduleName==='assetInfo_drilling'){
      state.drillData.queryDisplayFlag = false;
    } else{
      state.drillData.queryDisplayFlag = true;
    } 
...
const actions = {
  async  loadDrillTableData({commit,state}){
      ...
      state.parameters.p1 = state.drillData.queryParams.p1;
      state.parameters.p2 = state.drillData.queryParams.p2;
      state.parameters.p3 = state.drillData.queryParams.p3;
      let gridData = await common.req(state.parameters.detailUrl, state.parameters);
