<template>
  <div class="el-panel-bigscreen">
    <div class="el-panel-bigscreen-heading">
      <h3 class="el-panel-bigscreen-title"><i class="el-icon-caret-right title-symbol"></i>重要资产受威胁TOP5</h3>
    </div>
    <div class="el-panel-bigscreen-body">
      <div class="echartsbox10" id="keyassettop5"></div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
     getInitData() {
      this.common.req('/ssa/screenExhibit/getEventTypeTop5ByKey.do', {dateRange: 'week'}, this).then((data) => {
        this.loadMapChart(data);
      })
    },
    loadMapChart(data) {
      let myChart = echarts.init(document.getElementById('keyassettop5'));
      var lenData = [];
      var seriesData=[];
      var xAxis = [];
      var MyXAxis = [];
