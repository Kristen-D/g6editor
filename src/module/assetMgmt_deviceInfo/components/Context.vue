...
<div class="whitebackground Whitespace">
    <div class="top-title">设备信息管理</div>

    <div :class="[advancedFlag ? 'senior-search':'ordinary-search', clearfloat]">
        <el-form ref="params" :model="params" label-width="80px">
          <div class="" style="display:inline-block;">
            <el-form-item label="设备ID" style="display:inline-block;">
              <el-input v-model="params.devId" placeholder="设备ID"></el-input>
            </el-form-item>
            <el-form-item label="设备名称" style="display:inline-block; margin-left:20px;">
              <el-input v-model="params.devName" placeholder="设备名称"></el-input>
            </el-form-item>
            <el-form-item style="display:inline-block; margin-left:20px;" v-if="!advancedFlag">
          ...
          <transition  name="slide-fade" >
            <div class="" v-if="advancedFlag">
              <el-form-item label="产品名称" style="display:inline-block;">
                <el-input v-model="params.productName" placeholder="产品名称"></el-input>
              </el-form-item>
              <el-form-item label="设备版本" style="display:inline-block; margin-left:20px;">
                <el-input v-model="params.devVersion" placeholder="设备版本"></el-input>
              </el-form-item>
              <el-form-item label="设备描述" style="display:inline-block; margin-left:20px;">
                <el-input v-model="params.devDesc" placeholder="设备描述"></el-input>
              </el-form-item>
            </div>
          </transition>
  ...
  <el-dialog title="导入" :visible.sync="exportDialogShow" width="30%">
    <div class="dialog-content">
      ...
      <a href="/static/template/DeviceInfo.xlsx" class="dialog-importEvent-label">设备信息模版</a>

      <el-upload ref="upload" action="/ssa/assetMgmt/batchImportDeviceInfo.do" :auto-upload="false" :multiple="false" :on-success="handleSuccess">
...
<script>
import editTable from './DeviceInfoEditTable.vue'
export default {
  components: {
    editTable,
  },
  ...
  methods: {
    ...
    async delCheckedRows() {
      ...
      this.$confirm('此操作将删除选中条目, 是否继续?', '提示', {
        ...
        var devIds = [];
        rows.forEach(row => {
          devIds.push(row.devId);
        })
        var result = await this.common.reqBackState("/ssa/assetMgmt/batchedDetleDeviceInfo.do", {
          "devIds": devIds
        }, this);
        if(result.state) {
          this.$store.dispatch('reloadTable', this.tableId);
