...
export default {
  ...
  methods: {
    ...
    handleEdit(i, row) {
      row.tableId = this.id;
      this.$router.push({name:'deviceInfoForm', params:row })
      // this.$store.dispatch('handleEdit', [this.id, i, row]);
    },
    handleDelete(i, row) {
      this.$store.dispatch('handleDelete', [this, i, {"devIds":[row.devId]}]);
    },
    saveEdit(i, row) {
  ...
  created: function() {
    var defConf = {
      ...
      multipleSelection: [],
      //默认ajax属性
      tableUrl: "/ssa/assetMgmt/queryDeviceInfo.do",
      params: {},
      saveUrl: "/ssa/assetMgmt/addOrModifyDeviceInfo.do",
      delUrl: "/ssa/assetMgmt/batchedDetleDeviceInfo.do",
    };
