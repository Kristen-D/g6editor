  ...
  <div class='row'>
    <el-table :data="ruleList" :border="false" tooltip-effect="dark" style="width: 100%" :default-sort="{prop: 'ruleId', order: 'ascending'}"
      ...
      <el-table-column v-for="col in showHeadCols" :prop="col.prop" :label="col.label" :formatter="formatter"
        :class-name="col.linkclass" :width="col.width" :show-overflow-tooltip="col.overflowTooltip">
      </el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      handleCopy(index, row) {
        ...
        commonAjax.req('/ssa/ruleManage2/copyRule.do', param).then((data) => {
          this.getRuleList();
        })
      ...
      handleStop(index, row) {
        var param = {};
        param.ruleId = row.ruleId;
        commonAjax.req('/ssa/ruleManage2/stopRule.do', param).then((data) => {
          this.getRuleList();
        })
      },
      handleStart(index, row) {
        var param = {};
        param.ruleId = row.ruleId;
        commonAjax.req('/ssa/ruleManage2/startRule.do', param).then((data) => {
          this.getRuleList();
        })
      },
