...
<el-form ref="formRule" :model="formRule"  :rules="rules" label-width="120px">
       <template  v-if="formRule.configTemplate==3">
            <el-row >
                <div class="el-sub-panel-heading"><h4 class="sub-title">分组统计</h4></div>
            </el-row>
            <el-row>
                    <el-col :span="5">
                        <el-form-item label="时间属性：" prop="stat.timeField" :rules="{type:'string',required: true, message: '时间属性不能为空', trigger: 'change'}">
                            <el-select v-model="formRule.stat.timeField">
            ...
            <el-row>
                <el-col :span="5">
                    <el-form-item label="时间窗口："  prop="stat.timeWindow" :rules="{type:'number',required: true, message: '时间窗口必须为整数', trigger: 'blur'}">
                        <el-input v-model.number="formRule.stat.timeWindow" @change="secTimeWindow"></el-input>
                    </el-form-item>
                </el-col>
                <el-col :span="3">
                        <el-select v-model="formRule.stat.timeUnit" @change="secTimeWindow">
            ...
            <el-row>
                    <el-col :span="5">
                        <el-form-item label="输出时间点：" prop="stat.outputTime" :rules="{required: true, message: '输出时间点不能为空', trigger: 'change'}">
                            <el-select v-model="formRule.stat.outputTime">
                                  <el-option v-for="item in outputTimeList"
                                      :key="item.value"
                                      :label="item.name"
                                      :value="item.value">
                                  </el-option>
            ...
            <template  v-if="formRule.configTemplate==3 ||(formRule.configTemplate==2&&(formRule.relationType==2))">
                <el-row>
                    <el-col :span="13">
                        <el-form-item label="分组条件：" prop="stat.groupField">
                                <el-select v-model="formRule.stat.groupField" multiple placeholder="多选" @change="groupFieldChange">
                                    <el-option
                ...
                <el-form-item label="分组类型：">
                    <el-radio-group v-model="formRule.stat.groupType">
                        <el-radio :label="1">累加(sum)</el-radio>
                        <el-radio :label="2">计数(count)</el-radio>
                        <el-radio :label="3">连续性</el-radio>
                        <el-radio :label="4">唯一计数</el-radio>
                    </el-radio-group>
                </el-form-item>
                <el-row>
                    <el-col :span="5" v-show="formRule.stat.groupType==1 || formRule.stat.groupType==3">
                        <el-form-item prop="stat.sumField" v-if="formRule.stat.groupType==1 || formRule.stat.groupType==3">
                            <el-select v-model="formRule.stat.sumField" placeholder="计算字段" >
                                <el-option v-for="item in formRule.inputEventRules[0].attrData"
                                    :key="item.attrName"
                                    :label="item.attrDesc"
                                    :value="item.attrName">
                    ...
                    <el-col :span="5" v-show="formRule.stat.groupType==4">
                        <el-form-item prop="stat.distinctField">
                            <el-select v-model="formRule.stat.distinctField" multiple placeholder="计算字段,可多选"  >
                                <el-option v-for="item in formRule.inputEventRules[0].attrData"
                                    :key="item.attrName"
                                    :label="item.attrDesc"
                                    :value="item.attrName">
                    ...
                    <el-col :span="4"  v-if="formRule.stat.groupType==3" >
                        <el-form-item  label-width="0px">
                              <el-input placeholder="值"  v-model="formRule.stat.stateValue"></el-input>
                        </el-form-item>
                    ...
                    <el-col :span="3">
                        <el-form-item label-width="0px" prop="stat.compareFunc">
                            <el-select v-model="formRule.stat.compareFunc" >
                    ...
                    <el-col :span="4">
                        <el-form-item label-width="0px" prop="stat.compareValue">
                            <el-input v-model="formRule.stat.compareValue"></el-input>
                        </el-form-item>
                    </el-col>
                </el-row>
            </template>
...
export default{
    ...
    methods:{
        secTimeWindow:function(){
                var sec=0;
                var unit=this.formRule.stat.timeUnit;
                var unitSec=0;
                if(unit=='s'){
                    unitSec=1;
                }else if(unit=='m'){
                ...
                sec=unitSec*this.formRule.stat.timeWindow;
                this.formRule.stat.secTimeWindow=sec;
