<template>
  <big-screen-grid-header
      ...
      <el-dialog :title="detailTitle" :visible.sync="myShow" width="85%" @close="onCloseIp()"  top="15%" append-to-body>
...
<script>
  ...
  export default {
    ...
    methods: {
          ...
          chartClick (params) {
            this.showDetail(params.data[0],params.seriesIndex,3);//日期，(已处置0,未处置1),高危
          },
          showDetail(dataTime,dataType,dataType2){
            this.detailTitle=""+dataTime+"的高危"+this.unitOptions[dataType].label+"告警详情";
            ...
            this.params.dataType2=dataType2;
            this.myShow = true;
    ...
    computed: {
      option () {
        return this.$store.state.context.highAlarm;
      },
      total () {
        return `(${this.$store.state.context.alarmSmallTitle}: ${this.$store.state.context.highAlarmTotal}条)`;
      }
    }
  }
</script>
