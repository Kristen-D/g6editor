<template>
  <div class="dialog-import-content" style="padding-bottom: 30px;">
     <div class="details-attack-one" style="margin-top:-20px;" id="detailId1">
      ...
      <table class="dialog-table" :width="width1">
        <tbody id="tbody1" v-html="tbodyStr1">
        </tbody>
      </table>
    </div>
     <div class="details-attack-two" id="detailId2">
      <div class="dialog-subtitle">事件信息</div>
        <table class="dialog-table" :width="width2">
          <tbody id="tbody2" v-html="tbodyStr2">
          </tbody>
      </table>
     </div>
     <div class="details-attack-three" id="detailId3">
      <div class="dialog-subtitle">设备信息</div>
        <table class="dialog-table" :width="width3">
          <tbody id="tbody3" v-html="tbodyStr3">
        </tbody>
      </table>
     </div>
  ...
  import {mapState} from 'vuex'
    export default {
      ...
      computed:{
        ...mapState({
           params:state=>state.detail.params,
           eventTypeList:state=>state.detail.eventTypeList,
           ProductTypeList:state=>state.detail.ProductTypeList,
           EventThreatList:state=>state.detail.EventThreatList,
           EventGradeList:state=>state.detail.EventGradeList,
           EventRiskList:state=>state.detail.EventRiskList,
       })},
       watch:{
        tableData:function(tableData){
          console.log('tbodyStr1');
    ...
    append2(){
      ...
      {"lname":"资产名(源IP)","name":"asset_name_src"},
      {"lname":"所属机构(源IP)","name":"org_name_src"},
      {"lname":"资产负责人(源IP)","name":"owner_src"},
      {"lname":"使用人(源IP)","name":"asset_user_src"},
      {"lname":"资产外部IP(源IP)","name":"assert_out_ip_src"},

      {"lname":"目标IP","name":"dstIp_s"},
      {"lname":"目标地点","name":"event_dest_ip_area"},
      {"lname":"目标端口","name":"dstPort_s"},

      {"lname":"资产名(目标IP)","name":"asset_name"},
      {"lname":"所属机构(目标IP)","name":"org_name"},
      {"lname":"资产负责人(目标IP)","name":"owner"},
      {"lname":"使用人(目标IP)","name":"asset_user"},
      {"lname":"资产外部IP(目标IP)","name":"assert_out_ip"},
...
<style>
/*攻击事件弹出框*/
.dialog-subtitle{ border-left: 3px solid #007BE8; padding-left: 10px; line-height: 20px; font-weight: 600; margin: 20px auto;letter-spacing: 1px;}
table.dialog-table{ border-collapse:collapse;color:#fff!important }
table.dialog-table >tbody> tr > td{ border:  1px solid #e1e3e4; padding: 10px; line-height: 20px;}
table.dialog-table >tbody> tr > td:nth-child(odd){  text-align: center; font-weight: 600; letter-spacing: 1px; width: 12%;}
table.dialog-table >tbody> tr > td:nth-child(even){ padding-left: 2%; width:16% }
table.dialog-table >tbody> tr > td.dialog-attack-number{ font-family: "Haettenschweiler"; font-size: 48px; color: #03a182;}
