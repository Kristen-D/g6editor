...
async  loadTableData({commit,state},[id,params,url]){
    ...
    var result = await common.reqBackState(url,params);
    result.data.rowData.forEach((m) => {
      let a = '';
      let b = [];
      m.reports.forEach((r) => {
        a = a+r.name+'，';
      ...
      m.attachment = a.substring(0, a.length-1);
      m.attachments = b;
    });
    commit('setTableAfterLoad',[id,result]);
   ...
   var id = context.id
     context.$confirm('此操作将删除该行, 是否继续?', '提示', {
       ...
       type: 'warning'
        ...
        var result = await common.requestWithComplexContent(state[id].delUrl, row, context);
        if(result.state) {
