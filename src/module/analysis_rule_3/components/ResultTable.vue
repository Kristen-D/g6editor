<template>
    <div class='row'>
        <el-table :data="ruleList" :border="false" tooltip-effect="dark"  style="width: 100%"  :default-sort = "{prop: 'ruleId', order: 'ascending'}"  @selection-change="handleSelectionChange" @cell-click="cellClick">
                <el-table-column type="selection"  width="60"></el-table-column>

                <el-table-column 
                    v-for="col in showHeadCols"
                    :prop="col.prop" :label="col.label"  :formatter="formatter" :class-name="col.linkclass" :width="col.width">
                </el-table-column>
                ...
                <el-table-column label="操作" width="180">
                    <template scope="scope">
                        <el-button
                        type="text"
                        @click="handleEdit(scope.$index, scope.row)"><i class="fa fa-edit fa-lg"></i></el-button>
                        <el-button
                        type="text"
                        @click="handleDelete(scope.$index, scope.row)"><i class="fa fa-close fa-lg"></i></el-button>
                        <el-button
                        type="text"
                        @click="handleCopy(scope.$index, scope.row)"><i class="fa fa-copy fa-lg"></i></el-button>
                        ...
                        type="text"
                        @click="handleStart(scope.$index, scope.row)">启用</i></el-button>
                    </template>
                </el-table-column>
        </el-table>
    ...
    export default {
        ...
        methods:{
            ...
            handleEdit(index,row){
                ...
                commonAjax.req('/ssa/ruleManage3/getRuleJsonById.do',param).then((data)=>{
            ...
            handleCopy(index,row){
                var param={};
                param.ruleId=row.ruleId;
                commonAjax.req('/ssa/ruleManage3/copyRule.do',param).then((data)=>{
                      this.getRuleList();
                })
            ...
            handleStop(index,row){
                var param={};
                param.ruleId=row.ruleId;
                commonAjax.req('/ssa/ruleManage3/stopRule.do',param).then((data)=>{
                       this.getRuleList();
                })
            },
            handleStart(index,row){
                var param={};
                param.ruleId=row.ruleId;
                commonAjax.req('/ssa/ruleManage3/startRule.do',param).then((data)=>{
                      this.getRuleList(); 
                })
            },
