...
const state = {
   ...
   ruleTotal:1,
   multipleSelection: [],
   checkHeadCols:['编号','设备类型','规则分类','规则名称','状态','时间窗口','是否告警'],
...
const actions={
  ...
  getRuleList:function(context){
      commonAjax.req('/ssa/ruleManage3/getRuleList.do',context.state.queryCondition,context).then((data)=>{
  ...
  getRuleById:function(context){
      ...
      commonAjax.req('/ssa/ruleManage3/getRuleById.do',params,context).then((data)=>{
  ...
  competeRule:function(context,rule){
      ...
      for(var relation of rule.relations){
                relation.attrData = [];        
                var nextIdentity='';
      ...
      if(rule.outputRules[0].whichCascader == undefined){
          ...
          console.log(context.state.eventTypeList.length)
          for(var group of context.state.eventTypeList){
              for(var item of group.children){
                  console.log(group.id+'  '+item.id +'  '+rule.outputRules[0].which)
          ...
          console.log("compete which:"+rule.outputRules[0].whichCascader)
      }
  ...
  deleteRuleById:function(context,val){
    ...
    commonAjax.req('/ssa/ruleManage3/delRuleById.do',params,context).then((data)=>{
  ...
  delMultiRule:function(context){
      ...
      if(select.length>0){
          ...
          commonAjax.req('/ssa/ruleManage3/delMultiRule.do',params,context).then((data)=>{
              context.commit('setMultipleSelection',[]);
              context.dispatch('getRuleList',context.state.queryCondition);      
          })
      }
  ...
  */
  ...
  getEventOutAttrValueList:function(context){
        commonAjax.req('/ssa/ruleManage3/getEventOutAttrValueList.do').then((data)=>{
                    context.commit('setEventOutAttrValueList',data); 
        }) 
  },
  getEventOutAttrList:function(context){
        commonAjax.req('/ssa/ruleManage3/getEventOutAttrList.do').then((data)=>{
