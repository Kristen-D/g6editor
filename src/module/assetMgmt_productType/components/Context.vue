...
<div class="whitebackground Whitespace">
    <div class="top-title">产品类型管理</div>

    <div :class="[advancedFlag ? 'senior-search':'ordinary-search', clearfloat]">
        <el-form :model="params" ref="params" label-width="100px">
          <div class="" style="display:inline-block;">
            <el-form-item label="产品类型ID" style="display:inline-block;">
              <el-input v-model="params.productType" placeholder="产品类型ID"></el-input>
            </el-form-item>
            <el-form-item label="产品类型名称" style="display:inline-block;margin-left:20px;">
              <el-input v-model="params.typeName" placeholder="产品类型名称"></el-input>
            </el-form-item>
            ...
            <div class="" v-if="advancedFlag">
              <el-form-item label="父产品ID" style="display:inline-block;">
                <el-input v-model="params.parentId" type="text" name="" placeholder="父产品ID"></el-input>
              </el-form-item>
              <el-form-item label="类型级别" style="display:inline-block; margin-left:20px;">
                <el-select v-model="params.typeLevel" placeholder="请选择级别" style="width:200px;">
  ...
  <el-dialog title="导入" :visible.sync="exportDialogShow" width="30%">
    <div class="dialog-content">
      ...
      <a href="/static/template/ProductType.xlsx" class="dialog-importEvent-label">产品类型模版</a>

      <el-upload ref="upload" action="/ssa/assetMgmt/batchImportProductType.do" :auto-upload="false" :multiple="false" :on-success="handleSuccess">
  ...
  <el-dialog title="自定义列" :visible.sync="customColShow" width="30%">
    <div class="dialog-content">
      <p>
        页面上最多可以选择4个字段，已选{{customColumnList.length}}个
        <span @click="recoverColumns" class="dialog-reset">恢复默认设置</span>
      </p>
      <el-checkbox-group v-model="customColumnList" :min="3" :max="4" style="padding-top:10px; font-size:14px;">
...
<script>
import editTable from './ProductTypeEditTable.vue'
export default {
  components: {
    editTable,
  },
  ...
  methods: {
    ...
    async delCheckedRows() {
      ...
      this.$confirm('此操作将删除选中条目, 是否继续?', '提示', {
        ...
        var productTypes = [];
        rows.forEach(row => {
          productTypes.push(row.productType);
        })
        var result = await this.common.reqBackState("/ssa/assetMgmt/batchedDetleProductType.do", {
