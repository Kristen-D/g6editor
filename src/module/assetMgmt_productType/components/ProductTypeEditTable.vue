...
<div class="">
  ...
  <el-table :data="defConf.tableData" style="width: 100%" @selection-change="handleSelectionChange">
    ...
    <el-table-column :prop="key" :label="value" v-for="(value, key) in columnList"  :align="center">
    </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleEdit(i, row) {
      row.tableId = this.id;
      this.$router.push({name:'productTypeForm', params:row })
      // this.$store.dispatch('handleEdit', [this.id, i, row]);
    },
    handleDelete(i, row) {
      this.$store.dispatch('handleDelete', [this, i, {"productTypes":[row.productType]}]);
    },
    saveEdit(i, row) {
  ...
  created: function() {
    var defConf = {
      ...
      multipleSelection: [],
      //默认ajax属性
      tableUrl: "/ssa/assetMgmt/queryProductType.do",
      params: {},
      saveUrl: "/ssa/assetMgmt/addOrModifyProductType.do",
      delUrl: "/ssa/assetMgmt/batchedDetleProductType.do",
    };
