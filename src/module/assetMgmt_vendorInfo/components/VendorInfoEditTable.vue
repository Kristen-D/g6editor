...
export default {
  ...
  methods: {
    ...
    handleEdit(i, row) {
      row.tableId = this.id;
      this.$router.push({name:'vendorInfoForm', params:row })
      // this.$store.dispatch('handleEdit', [this.id, i, row]);
    },
    handleDelete(i, row) {
      this.$store.dispatch('handleDelete', [this, i, {"vendorIds":[row.vendorId]}]);
    },
    saveEdit(i, row) {
  ...
  created: function() {
    var defConf = {
      ...
      multipleSelection: [],
      //默认ajax属性
      tableUrl: "/ssa/assetMgmt/queryVendorInfo.do",
      params: {},
      saveUrl: "/ssa/assetMgmt/addOrModifyVendorInfo.do",
      delUrl: "/ssa/assetMgmt/batchedDetleVendorInfo.do",
    };
