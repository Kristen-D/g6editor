...
<div class="whitebackground Whitespace">
    <div class="top-title">设备厂商管理</div>

    <div :class="[advancedFlag ? 'senior-search':'ordinary-search', clearfloat]">
        <el-form :model="params" ref="params" label-width="90px">
          <div class="" style="display:inline-block;">
            <el-form-item label="供应商ID" style="display:inline-block;">
              <el-input v-model="params.vendorId" placeholder="供应商ID"></el-input>
            </el-form-item>
            <el-form-item label="供应商名称" style="display:inline-block;margin-left:20px;">
              <el-input v-model="params.vendorName" placeholder="供应商名称"></el-input>
            </el-form-item>
            <el-form-item label="供应商描述" style="display:inline-block;">
              <el-input v-model="params.vendorDesc" placeholder="供应商描述"></el-input>
            </el-form-item>
  ...
  <el-dialog title="导入" :visible.sync="exportDialogShow" width="30%">
    <div class="dialog-content">
      ...
      <a href="/static/template/VendorInfo.xlsx" class="dialog-importEvent-label">设备厂商模版</a>

      <el-upload ref="upload" action="/ssa/assetMgmt/batchImportVendorInfo.do"
...
<script>
import editTable from './VendorInfoEditTable.vue'
export default {
  components: {
    editTable,
  },
  ...
  methods: {
    ...
    async delCheckedRows() {
      ...
      this.$confirm('此操作将删除选中条目, 是否继续?', '提示', {
        ...
        var vendorIds = [];
        rows.forEach(row => {
          vendorIds.push(row.vendorId);
        })
        var result = await this.common.reqBackState("/ssa/assetMgmt/batchedDetleVendorInfo.do", {
          "vendorIds": vendorIds
        }, this);
        if(result.state) {
          this.$store.dispatch('reloadTable', this.tableId);
