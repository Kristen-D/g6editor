<template>
  <div>
    <div class="container-wrap">
      <el-row :gutter="24">
        <el-col :span="7">
          <high-alarm-trend style="height: 229px;"></high-alarm-trend>
          <middle-alarm-trend style="height: 229px;"></middle-alarm-trend>
          <attack-trend style="height: 229px;"></attack-trend>
          <threated-trend style="height: 229px;"></threated-trend>
        </el-col>
        <el-col :span="17">
    ...
    methods: {
      ...
      refreshData() {
        if(!this.$store.state.context.isSearch){
          var d = new Date();
          //this.$store.state.context.params.timeValue=d.getHours()+1;
          this.$store.state.context.params.timeValue=12;
          this.$store.state.context.params.timeType=1;
        }
        this.$store.dispatch(types.GET_ALL_DATA);
        this.$store.state.context.date = this.getCurrentDate();
        setTimeout(() => {
          this.refreshData();
        }, 60000);
      },
...
<style >
  ...
  .day_time{ text-align: right; margin-top: -50px; margin-right: 30px;  color:#99cae8; font-size: 20px; cursor: pointer;}
