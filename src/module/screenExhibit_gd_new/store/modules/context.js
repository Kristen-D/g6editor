...
import {
  highAlarmTrend, middleAlarmTrend,
  attackTrend, threatedTrend, attackPie,
  attackBar, worldAttackMap
...
const MAP_TYPE = ['world', 'guangdong'];

const HIGH = 3, MIDDLE = 2;
const FINISHED = 3, UNFINISHED = 2;
...
const getAlarmInfoUrl = '/ssa/screenExhibit/getAlarmInfo_new.do';
...
const mutations = {
  ...
  [types.GET_ASSET_NAME](state, data) {
    state.deviceList = data[0].map(value => {
      return {
        ...
        name: value.event_dest_ip,
        event_dest_ip:value.event_dest_ip
      }
    })
  },
  [types.GET_EVENT_DST_IP](state, data) {
    if (state.srcIpList.result.length > 5) {
      state.srcIps = state.srcIpList.result.slice(0, 5);
    } else {
    ...
    state.dstIpList = state.srcIps.map(value => {
       return {
  ...
  [types.GET_EVENT_SRC_IP](state) {
    ...
    state.mapChart.series[0].data = result[1];
    state.mapChart.series[1].data = result[0];
  ...
  [types.SET_EVENT_SRC_BAR](state) {
    ...
    state.attackPie.series[0].data = res.slice(0, 10).reverse();
  },
  [types.GET_GEO_COORD_MAP](state, data) {
    state.geoData = data;
  },
  [types.ON_SMALL_MAP_CHANGE](state, {type, dispatch, commit}) {
    switch (type) {
      case MAP_TYPE[0]:
...
const actions = {
  async [types.EVENT_THREAT_LIST]({commit}) {
  ...
  async [types.GET_LINE_BY_EVENT_TYPE](context) {
    let data = await commonAjax.req(getLineByEventTypeUrl + '?dateRange=month', {externalIp_s: state.params.externalIp_s, dateRange: 'month'}, this);
    let data2 = await commonAjax.req(getLineByEventTypeUrl + '?dateRange=week', {externalIp_s: state.params.externalIp_s, dateRange: 'week'}, this);
    // let data = {
  ...
  async [types.GET_EVENT_SRC_IP](context) {
    ...
    state.ipList=state.srcIpList.ipList;
    context.commit(types.GET_EVENT_DST_IP);
