...
<el-form ref="formRule" :model="formRule"  :rules="rules" label-width="120px">
    <el-row>
        ...
        <el-row v-for="(outputRule,index) in formRule.outputRules">
            <el-col :span="11">
                <el-form-item v-bind:label="'字段'+(index)+'：'">
                    ...
                    <el-select v-model="outputRule.outputAttrName" @change="outAttrChange(outputRule)">
            ...
            <el-col :span="4">
                <el-form-item label="取值" label-width="60px" v-if="　outputRule.type == 0">
                    <el-input v-model="outputRule.which"></el-input>
                </el-form-item>
                <el-form-item label="取值" label-width="60px" v-if="　outputRule.type==1">
                ...
                <el-form-item label="取值" label-width="60px" v-if="outputRule.type==2">
                    <el-select v-model="outputRule.which">
...
export default{
    ...
    methods:{
        addOutputRule:function(){
            ...
            changeAttr:function(outputRule){
                //获取attr_type
                var attrId='';
                outputRule.attrType=0;
                for(var item of this.attrData){
                    if(item.attrName==outputRule.attrName){
                        outputRule.attrType=item.attrType;
                        attrId=item.attrId;
                        break;
                    }
                }

                if(outputRule.attrType==1){
                    for(var item of this.attrParamList){
                        if(outputRule.attrName==item.attrName){
                            outputRule.paramData.push(item)
                        }
                    }
                }
            },
        outAttrChange:function(outputRule){
                ...
                var attr = outputRule.outputAttrName
                outputRule.which=''
                if(attr[1]=='.'){
                    outputRule.type=2
                }else{
