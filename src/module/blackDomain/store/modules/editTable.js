   ...
   var id = context.id
     context.$confirm('此操作将删除该行, 是否继续?', '提示', {
       ...
       type: 'warning'
        ...
        var result = await common.reqBackState(state[id].delUrl, row, context);
        if(result.state) {
          state[id].tableData.splice(i, 1);
          context.$message({ type: 'success', message: result.message});
        }else {
          context.$message({ type: 'error', message: result.message});
        }
     })
 },
 async handleDeleteShow({commit,state,dispatch},[context,i,row]){
   var id = context.tableId
     context.$confirm('此操作将删除该行, 是否继续?', '提示', {
       confirmButtonText: '确定',
       cancelButtonText: '取消',
       type: 'warning'
...
async saveEdit({commit,state,dispatch},[context,i,row]){
 ...
 async  initDataStatus({commit,state,dispatch},[tableData]){
     var arrStatus = tableData;
     for(let v of arrStatus){
       let data = await common.reqBackState('/ssa/monitorTool/getPingTestResult.do', v);
       data.data = data.data?1:0;
       if(data.data!=v.status){
         v.status=data.data;
         let data2 = await common.reqBackState('/ssa/monitorTool/addOrModifyToolInfo.do', v);
       }
     }
   }
