<template>
<div class="">
    <div class="top-title">报表管理</div>
    <div :class="[advancedFlag ? 'senior-search':'ordinary-search', clearfloat]">
        <el-form :model="params" ref="params" label-width="100px">
          <div class="" style="display:inline-block;">
            <el-form-item label="模版名称" style="display:inline-block;">
              <el-input v-model="params.templateName" placeholder="模版名称" style="width:220px;"></el-input>
            </el-form-item>
  ...
  <el-dialog title="导入" :visible.sync="exportDialogShow" width="30%">
    <div class="dialog-content">
      ...
      <a href="/ssa/assetMgmt/downloadVendorTemplate.do" class="dialog-importEvent-label">下载事件模版</a>
...
<script>
import editTable from './ReportTemplateEditTable.vue'
export default {
  components: {
    editTable,
  },
  ...
  methods: {
    ...
    async delCheckedRows() {
      ...
      this.$confirm('此操作将删除选中条目, 是否继续?', '提示', {
        ...
        var result = await this.common.reqBackState("/ssa/reportTemplateManage/deleteTemplate.do", {
          "ids": ids
        }, this);
        if(result.state) {
          this.$store.dispatch('reloadTable', this.tableId);
