...
<div class="whitebackground Whitespace">
  <div class="top-title">产品信息管理</div>
    ...
    <div :class="[advancedFlag ? 'senior-search':'ordinary-search', clearfloat]">
        <el-form :model="params" ref="params" label-width="90px">
          <div  style="display:inline-block;">
            ...
            <el-form-item label="产品型号" style="display:inline-block;margin-left:20px;">
              <el-input v-model="params.productModel" type="text" name="" placeholder="产品型号"></el-input>
            </el-form-item>
          ...
          <transition enter-active-class="animated fadeInRight" leave-active-class="animated fadeOutLeft" mode="out-in">
            <div class="" v-if="advancedFlag">
              <el-form-item label="产品类型" style="display:inline-block;">
                <!-- <el-input v-model="params.typeName" placeholder="产品类型"></el-input> -->
              ...
              <el-form-item label="产品供应商" style="display:inline-block; margin-left:20px;">
                <el-input v-model="params.vendorName" placeholder="产品供应商"></el-input>
              </el-form-item>
            </div>
          </transition>
  ...
  <el-dialog title="导入" :visible.sync="exportDialogShow" width="30%">
    <div class="dialog-content">
      ...
      <a href="/static/template/ProductInfo.xlsx" class="dialog-importEvent-label">产品信息模版</a>

      <el-upload ref="upload" action="/ssa/assetMgmt/batchImportProductInfo.do" :auto-upload="false" :multiple="false" :on-success="handleSuccess">
  ...
  <el-dialog title="自定义列" :visible.sync="customColShow" width="30%">
    <div class="dialog-content">
      <p>
        页面上最多可以选择6个字段，已选{{customColumnList.length}}个
        <span @click="recoverColumns" class="dialog-reset">恢复默认设置</span>
      </p>
      <el-checkbox-group v-model="customColumnList" :min="3" :max="6" style="padding-top:10px; font-size:14px;">
...
<script>
import editTable from './ProductInfoEditTable.vue'
export default {
  components: {
    editTable,
  },
  ...
  methods: {
    ...
    async delCheckedRows() {
      ...
      this.$confirm('此操作将删除选中条目, 是否继续?', '提示', {
        ...
        var result = await this.common.reqBackState("/ssa/assetMgmt/batchedDetleProductInfo.do", {
