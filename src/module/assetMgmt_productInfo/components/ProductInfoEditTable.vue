...
export default {
  ...
  methods: {
    ...
    handleEdit(i, row) {
      row.tableId = this.id;
      this.$router.push({name:'productInfoForm', params:row })
      // this.$store.dispatch('handleEdit', [this.id, i, row]);
    },
    handleDelete(i, row) {
      this.$store.dispatch('handleDelete', [this, i, {"productIds":[row.productId]}]);
    },
    saveEdit(i, row) {
  ...
  created: function() {
    var defConf = {
      ...
      multipleSelection: [],
      //默认ajax属性
      tableUrl: "/ssa/assetMgmt/queryProductInfo.do",
      params: {},
      saveUrl: "/ssa/assetMgmt/addOrModifyProductInfo.do",
      delUrl: "/ssa/assetMgmt/batchedDetleProductInfo.do",
    };
