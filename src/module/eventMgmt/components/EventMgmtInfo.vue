  ...
  <el-row>
    <div class="el-sub-panel-heading">
      ...
      <div class="float-r prostate">
        当前状态： <span>{{this.$route.params.state}}</span> <i v-show="this.$route.params.status != 0 " class="el-icon-view" style="color:#20a0ff" @click="showProcessTrace"></i>
      </div>
    ...
    <el-col :span="24">
      <el-collapse v-model="activeNames" @change="showTaskDeTail($event)" class="Check-the-process-state-collapse">
        <!-- 唯一标识 流程实例id,任务标识,任务id-->  <!-- item.taskDefinitionKey != 'eventAdjustTask'  -->
        <el-collapse-item class="processcollapse-item" v-for="(item, index) in hiTasks" v-show="item.taskDefinitionKey != 'submissionTask' && item.deleteReason != 'deleted'" :name="item.processInstanceId+','+item.taskDefinitionKey+','+item.id">
...
export default {
  ...
  data() {
    return {
      ...
      detailMap: {
        eventHandleInfoTask: "eventHandleInfoDetail",
        eventReportTask: "eventReportDetail",
        submissionTask: "submissionTaskDetail",
        eventAduitTask: "eventAduitDetail",
        eventAdjustTask: "eventHandleInfoDetail",
        inquisitionTask: "inquisitionDetail",
        companyReportTask: "companyReportDetail",
        superviseTaskReportTask: "superviseTaskReportDetail",
        investigateReportTask: "investigateReportDetail",
        superviseRectifyTask: "superviseRectifyDetail",
        feedbackTask: "feedbackDetail",
        feedbackCheckTask: "feedbackCheckDetail",
        eventtaskAssessmentTask: "eventtaskAssessmentDetail",
        administrativePenaltyTask: "administrativePenaltyDetail"
      }
    };
  },
  created: function() {
    ...
    this.utils.req("/ssa/eventMgmt/listHiTask.do", {
      ...
      this.hiTasks.push({
        name: "事件处置详单",
        taskDefinitionKey: "eventHandleInfoTask",
        processInstanceId: processInstanceId,
        id: params.event_handling_id,
    ...
    async showTaskDeTail(activeNames, e) {
      ...
      for (id in activeNames) {
        ...
        if (this.store[task] == undefined) {
          ...
          params.event_handling_id = this.$route.params.event_handling_id;
          params.event_handling_header = this.$route.params.event_handling_header;
          params.orgName = this.$route.params.unit_name;
