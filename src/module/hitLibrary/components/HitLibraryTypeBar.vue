   ...
   data(){
    ...
    methods: {
      ...
      showAccountDetail(days){
        ...
        this.tableUrl="/ssa/hitLibrary/getLineHighRiskAccountDetail.do";
      ...
      async loadMapChart(){
        var data = await this.commonAjax.req("/ssa/hitLibrary/getRadarHitLibraryType.do", this.params);
        ...
        for(var i = 0; i < data.length; i++){
            var dataUnit = data[i];
            var child={ name: 'cookies撞库', max: 1000};
            child.name=dataUnit.v_type;
            indicatorData.push(child);
        };

        var child1={ name: '僵尸', max: 1000};
        var child2={ name: '突破验证码', max: 1000};
